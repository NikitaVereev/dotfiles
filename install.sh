...
OS=$(if [[ -f /etc/arch-release ]]; then echo arch
...
ARCH_PKGS=(hyprland hyprlock hyprpaper waybar swaync kitty tmux zsh starship
           zoxide fzf fd rofi yazi thunar fastfetch cliphist swww imagemagick
...
main() {
    ...
    echo
    [[ ! $REPLY =~ ^[Yy]$ ]] && { echo ""; info "Installation cancelled"; exit 0; }
